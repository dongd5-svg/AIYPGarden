...
document.getElementById('saveTaskBtn').onclick = async () => {
  ...
  if (editingTaskId) {
    const prevTask = tasksData[editingTaskId];
    await ref.doc(editingTaskId).update(payload);
    // Notify if newly assigned
    if (payload.assignedTo && payload.assignedTo !== prevTask?.assignedTo) {
      notifyTaskAssignedToUser(payload.assignedTo, payload.title);
    }
    showToast('Task updated!');
  } else {
    await ref.add({ ...payload, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    if (payload.assignedTo) notifyTaskAssignedToUser(payload.assignedTo, payload.title);
...
// ── Notify assignee (called when task is saved with assignedTo) ───
async function notifyTaskAssignedToUser(email, taskTitle) {
  if (!email || typeof sendNotification !== 'function') return;
  if (email === currentUser?.email) return; // Don't notify yourself
  try {
    ...
    await sendNotification({
      ...
      gardenName: currentGardenData?.name || '',
    });
  } catch {}
}
